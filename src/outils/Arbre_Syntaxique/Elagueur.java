...
public class Elagueur {
    ...
    private void ajoutCondition(Noeud_Non_Terminal noeud_if) {
        // Prend le dernier fils
        Noeud_A fils_condition = noeud_if.getEnfants().get(noeud_if.getEnfants().size() - 1);

        // Retire le dernier fils
        noeud_if.getEnfants().remove(fils_condition);

        // Crée un nouveau noeud condition
        Noeud_Non_Terminal condition = new Noeud_Non_Terminal(nonterminaux.get("£CONDITION"));

        // Ajoute le noeud condition au noeud if
        noeud_if.ajouterEnfant(condition);
        condition.setParent(noeud_if);

        // Ajoute le dernier fils au noeud condition
        condition.ajouterEnfant(fils_condition);
        fils_condition.setParent(condition);
    }

    private void ajoutCondition_boucle() {
        Stack<Noeud_A> pile = new Stack<>();
        ArrayList<Noeud_Non_Terminal> tag = new ArrayList<>();
        ...
        while (!pile.isEmpty()) {
            ...
            if (noeud instanceof Noeud_Non_Terminal) {
                ...
                if (nonterminaux.get("£IF")==((Noeud_Non_Terminal)noeud).getCode() ) {
                    tag.add((Noeud_Non_Terminal)noeud);
                }

                if (nonterminaux.get("£ELSIFEtoile")==((Noeud_Non_Terminal)noeud).getCode() ) {
                    tag.add((Noeud_Non_Terminal)noeud);
                }

                if (nonterminaux.get("£WHILE")==((Noeud_Non_Terminal)noeud).getCode() ) {
                    tag.add((Noeud_Non_Terminal)noeud);
                }
